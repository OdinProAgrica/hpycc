...
def test_make_url_request_1():
    current_row = 1
    chunk = 2
    result = interface.make_url_request(server, port, username, password, file_name, current_row, chunk, silent)
...
def test_parse_json_output_1():
    csv_file = False
    results = interface.make_url_request(server, port, username, password, '~a::test::file', 1, 6, silent)['Result']['Row']
...
def test_parse_json_output_2():
    csv_file = True
    results = interface.make_url_request(server, port, '~a::test::filecsv', 2, 6, username, password)['Result']['Row']
    result = interface.parse_json_output(results, column_names, csv_file, silent)
    assert result == {'a': ['3', '5', '7', '9', '11'], 'b': ['4', '6', '8', '10', '12']}
